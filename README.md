...
### أ) على سيرفر محلي (XAMPP) — الطريقة المستخدمة هنا

1. تثبيت [XAMPP](https://www.apachefriends.org/) وتشغيل **Apache** من لوحة التحكم
...
### ب) على استضافة خارجية (InfinityFree / cPanel وغيرها)
...
4. أنشئ `config.php` على الخادم وضع فيه مفتاحك (لا يُرفع مع المستودع).
5. تأكد أن الاستضافة تدعم **PHP 7.4+** مع تفعيل **cURL**.

> ⚠️ **الميكروفون يتطلب HTTPS.** متصفحات Chrome/Edge تسمح بـ Web Speech API فقط عبر
> `https://` أو `http://localhost`. على استضافة خارجية فعّل شهادة SSL، وإلا لن يعمل زر الميكروفون.

---

## 🐛 الخطوة 2: المشكلة في كود PHP وكيف حُلَّت
...
#### 1️⃣ مسار خاطئ — الواجهة تطلب ملفًا غير موجود (404)
...
#### 2️⃣ مسار `require` معطوب — خطأ قاتل (Fatal Error)
...
#### 3️⃣ مفتاح API فارغ لا يكتشفه الفحص
...
#### 4️⃣ `.htaccess` بصيغة Apache 2.2 المهجورة
...
> 🔒 `config.php` مُدرج في `.gitignore` ومحجوب بـ `.htaccess`، فلا يُرفع إلى GitHub
> ولا يُفتح من المتصفح. ارفع `config.example.php` فقط.
> إن سُرِّب مفتاح يومًا، ألغِه فورًا من AI Studio وأنشئ غيره.
...
2. افتح `http://localhost/chat_bot_exmample/` في **Chrome** أو **Edge**.
...
`SpeechRecognition` متاح في Chrome و Edge و Safari (بادئة `webkit`). **Firefox لا يدعمه**،
وفي هذه الحالة يعطّل التطبيق زر الميكروفون ويعرض رسالة توضيحية بدل الفشل الصامت.
